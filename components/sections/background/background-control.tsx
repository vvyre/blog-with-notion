...
export function BackgroundControl({
  ...
  const [[BODY_WIDTH, BODY_HEIGHT], setBodySize] = useState<[number, number]>([0, 0])
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (theme === 'light') Array.from(refs.entries()).map(([k, _]) => updated.set(k, 255))
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    IMG.onload = () => {
      ...
      targetLayouts.forEach(([k, top, right, bottom, left]) => {
        if (!k || top == null || right == null || bottom == null || left == null) return //해당하는 DOM이 없으면 로직 실행하지 않음
