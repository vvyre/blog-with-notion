...
export const useRandomIntState = (range: [number, number]): [number, () => void] => {
  ...
  const [num, setNum] = useState<number>(() => (typeof window === 'undefined' ? low : getRandomInt([low, high]) + low))

  const render = () => setNum(typeof window === 'undefined' ? low : getRandomInt([low, high]) + low)
