...
export function useScrollPosition(
  ...
  const updatePosition = () => {
    ...
    setPositionFromBody((scrollHeightRef ? scrollHeightRef.current?.scrollTop : BODY?.scrollTop) || 0);
  ...
  // 디바운싱시켜 스크롤 이벤트 등록
  const handleScroll = useDebouncedCallback(() => updatePosition(), scrollDebouncingDelay, [
    anchorRef,
    scrollHeightRef,
    BODY,
  ]);
